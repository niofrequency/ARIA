...
and i think we need to update the api/generate right?
...
      // Merge workflow and optional images into the RunPod input object
      const runpodInput: any = { workflow };

      if (images && Array.isArray(images)) {
        images.forEach((img: any) => {
          // This maps the image base64 to the filename key ComfyUI expects
          runpodInput[img.name] = img.image; 
        });
        console.log("📸 Image attached to workflow payload.");
      }
...
        console.error(`❌ RunPod Dispatch Error:`, errorText);
        return res.status(response.status).json({ error: 'RunPod Failed', details: errorText });
      }
...
    } catch (error: any) {
      console.error("❌ Neural Generation Proxy Failure:", error);
      return res.status(500).json({ error: error.message });
    }
  }
